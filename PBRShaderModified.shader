Shader "PBR/PBRShaderModified"
{
    Properties
    {
        _MainTex ("Albedo Map", 2D) = "white" {}
        _NormalMap ("Normal Map", 2D) = "bump" {}
        _MetOccRoughMap ("_MetOccRoughMap Map", 2D) = "black" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v_out i) : SV_Target
            {
                ...
                float metalness = saturate(max(_Metalness + EPS, tex2D(_MetOccRoughMap, uv)).r);
                //float occlusion = saturate(tex2D(_MetOccRoughMap, uv).g);
                float occlusion = saturate(max(_Occlusion + EPS, tex2D(_MetOccRoughMap, uv)).g);
                float roughness = saturate(max(_Roughness + EPS, tex2D(_MetOccRoughMap, uv)).b);
